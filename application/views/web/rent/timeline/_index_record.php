<?php
  $current_status_alert = $this->transaction->get_current_status_alert($record);
...
?>
<tr class="<?=$current_status_alert ? '' : 'bg-danger text-white'?>">
  <td><?=$number?></td>
  <th scope="row">
  ...
  <td class="text-center"><?=$this->transaction->get_purpose_desc($record)?></td>
  <td class="text-center"><?=$record->rent_long_day?> hari</td>
